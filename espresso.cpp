            ...
            for(int i = 0; i < pcn.size(); i++)
            {
                for(int j = 0; j < var; j++)
                {
                    for(int k = 0; k < comp.size(); k++)
                    {
                        ...
                        {
                            //if(sumb.size() > j)
                            // Building the blocking matrix variable by variable
                            sumb[j].first+=1;
...
//                }
                ...
                while(it->first != 0)
                {
                    ...
                    // Update the blocking matrix here
                    while(it != sumb.end())
                    {
                       if (it->first == 0)
                       {
...
int cost (vector < vector < bitset<2> > > temp)
{
...
vector < vector < bitset <2> > > espresso (vector < vector < bitset <2> > > temp )
{
    if(temp.empty() | temp.size()==1) return temp;

    vector < vector < bitset <2> > > off, ess, var1, var2;
    int cost1,cost2, c = 0;

    off = complement(temp);
    temp = expand(temp,off);
    temp = irredundant(temp);
    ess = essentials(temp);
    if(temp.empty()) return ess;
    cost1 = cost(temp);
    //cost2 = cost1;
    ...
    while(c < 3)
    {
        ...
        {
            cost2 = cost1;
            temp = reduce(temp,ess);
            temp = expand(temp,off);
            temp = irredundant(temp,ess);
            cost1 = cost(temp);
            if(cost1 < cost2 && c != 0) c = 0;

        } while(cost1 < cost2);
        c += 1;
    }
    temp.insert(temp.end(), ess.begin(), ess.end());
    return temp;
...
//    }
    ...
    //cout<<endl;
    //ktest = irredundant(ktest, vector < vector <bitset <2> > > (1,gen));
...
//    cout<<endl;
    ...
    ktest = espresso(expr);
    cout<<"Implicants of minimized o/p: "<<endl;
    cout<<var<<endl;
    cout<<ktest.size()<<endl;
    for(int l = 0; l < ktest.size(); l++)
    {
        for(int k = 0; k < ktest[0].size(); k++)
        {
            if(ktest[l][k] == 1) cout<<1;
            else if (ktest[l][k] == 2) cout<<0;
            else cout<<"-";
            //cout<<ktest[l][k]<<" ";
        }
        cout<<" 1";
